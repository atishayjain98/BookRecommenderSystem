...
for page in pages:
    ...
    for store in movie_data:
        ...
        gen = store.p.find("span", class_ = 'genre').text if store.p.find("span", class_ = 'genre') else "NA"
        genre.append(gen)
        ...
        dire=store.find('p',class_='').find_all('a')[0].text if store.find('p',class_='').find_all('a') else "NA"
...
movie_list.to_excel("movie_data_new.xlsx")